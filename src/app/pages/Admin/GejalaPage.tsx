...
export default function GejalaPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      if (id) {
        await setDoc(doc(db, "data-gejala", id), params);
      } else {
        await addDoc(collection(db, "data-gejala"), params);
      }
      getData();
      reset();
    } catch (e) {
  ...
  const handleDelete = async (data: any) => {
    const id = data?.id;
    try {
      await deleteDoc(doc(db, "data-gejala", id));
    } catch (e) {
      console.log(e);
    }
